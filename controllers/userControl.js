...
const loginPost = async (req, res) => {
  try {
    ...
    const user = await Users.findOne({ email: email }).then(
      async (userData) => {
        if (userData) {
...
const postDeleteProduct = (req, res) => {
  ...
  Carts.updateOne({ _id: cartid }, { $pull: { product: { productId: pid } } })
    .then(() => {
      // console.log(cart);
...
const postChangePasswod = async (req, res) => {
  try {
    ...
    Users.findOne({ user_id: uid })
      .then(async (result) => {
        const passwordMatch = await bcrypt.compare(password, result.password);
        if (passwordMatch) {
          if (password === currentPassword) {
            res.render("user/changePassword", {
              message: "old password and new pasword is same",
            });
          } else {
            Users.findOneAndUpdate({ user_id: uid }, { password })
              .then(() => {
...
const couponCheck = async (req, res) => {
  ...
  const check = await Coupons.findOne({ coupon_code: code });
  if (check) {
    let used = false;
  ...
  Users.findOne({ user_id: uid }).then(() => {
    Carts.aggregate([
      ...
      for (let i = 0; i < result.length; i++) {
        const sold =
          result[i].productDetail.soldCount + result[i].productQuantity;
        Products.updateMany(
          // eslint-disable-next-line no-underscore-dangle
...
const verifyPayment = (req, res) => {
  ...
  if (hmac == details.payment.razorpay_signature) {
    ...
    console.log("objId");
    Orders.updateOne({ _id: objId }, { $set: { paymentStatus: "Paid" } })
      .then(() => {
